...
(async () => {
  // minimap.js читает Camera как bare global внутри своих функций
  ...
  test("layout centers a tall bbox constrained by height", async () => {
    const Minimap = await loadMinimap();
  ...
  test("overflows is false without bounds", async () => {
    const Minimap = await loadMinimap();
    assert.equal(Minimap.overflows(null, { x: 0, y: 0, z: 1 }, 1200, 800), false);
  });

  test("overflows is false when the bbox fits the viewport at the current zoom", async () => {
    const Minimap = await loadMinimap();
    const b = { minX: 0, minY: 0, maxX: 400, maxY: 300 };
    assert.equal(Minimap.overflows(b, { x: 0, y: 0, z: 1 }, 1200, 800), false);
  });

  test("overflows is true when the bbox is wider than the viewport at the current zoom", async () => {
    const Minimap = await loadMinimap();
    const b = { minX: 0, minY: 0, maxX: 4000, maxY: 300 };
    assert.equal(Minimap.overflows(b, { x: 0, y: 0, z: 1 }, 1200, 800), true);
  });

  test("overflows accounts for zoom: zooming out can make an overflowing bbox fit", async () => {
    const Minimap = await loadMinimap();
    const b = { minX: 0, minY: 0, maxX: 4000, maxY: 300 };
    assert.equal(Minimap.overflows(b, { x: 0, y: 0, z: 0.2 }, 1200, 800), false);
  });

  test("viewportRect projects the visible world rect into minimap space", async () => {
    const Minimap = await loadMinimap();
  ...
  test("viewportRect follows a panned and zoomed camera", async () => {
    const Minimap = await loadMinimap();
    const map = { x: 0, y: 0, z: 0.1 };
