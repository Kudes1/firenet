...
async function loadCommon(store, opts = {}) {
  ...
  global.document = doc;
  global.window = { prompt: () => opts.promptResult, location: { reload() {} } };
  global.localStorage = {
...
test("the open-draft button creates a draft and stores its id", async () => {
  ...
  assert.equal(global.sessionStorage.getItem("firenet-draft-id"), "new-draft");
});
...
test("shows an editing banner with the draft's name and status, and a 'return to current' action", async () => {
  ...
  assert.equal(global.sessionStorage.getItem("firenet-draft-id"), null);
});

test("falls back to read-only if the active draft no longer exists", async () => {
  const { renderDraftBanner } = await loadCommon({ "firenet-draft-id": "gone" }, { draftMissing: true });
  let reloaded = false;
...
test("falls back to read-only if the active draft was merged in another tab", async () => {
  const { renderDraftBanner, localStore } = await loadCommon(
    {},
