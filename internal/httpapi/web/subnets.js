...
document.addEventListener("alpine:init", () => {
  Alpine.data("subnetsPage", () => ({
    rows: [], // {name, cidr, owner}
    loaded: false,
    saving: false,
    draft: { index: -1, name: "", cidr: "" },
    ...
    async init() {
      try {
        ...
        this.rows = (doc.subnets || []).map((s) => ({ name: s.name, cidr: s.cidr, owner: owner[s.name] || "" }));
        this.loaded = true;
    ...
    openAdd() {
      this.draft = { index: -1, name: "", cidr: "" };
      this.$refs.dialog.showModal();
    },

    openEdit(i) {
      this.draft = { index: i, name: this.rows[i].name, cidr: this.rows[i].cidr };
      this.$refs.dialog.showModal();
    },
    ...
    async saveDraft() {
      ...
      const entry = { name: this.draft.name.trim(), cidr: this.draft.cidr.trim() };
    ...
    async persist(next) {
      try {
        const doc = await Api.put("/api/subnets", { subnets: next.map(({ name, cidr }) => ({ name, cidr })) });
        const owner = {};
