...
const TopoScene = (() => {
  const { SVG_NS, DEVICE_W, DEVICE_H, NET_W, NET_H, UNION_COLORS, KINDS, el, center, linkOffsets, spreadOffset, pointAt, cloudSegs } = NetMap;
  const UNION_PAD = 30;
...
//   dim(obj)             — true, если элемент рисуется приглушённым;
  ...
  // buildScene собирает display list сцены для канвасного рендера: те же
  // слои и геометрия, что у render (unions → links → attaches → devices →
  // networks), но в виде примитивов {kind, id, ref, geom, style, ...}.
  // opts: theme (CanvasTheme), classes/mark/dim — как у render; fade —
  ...
  return Object.freeze({ ensureLayout, render, buildScene, bounds });
})();
