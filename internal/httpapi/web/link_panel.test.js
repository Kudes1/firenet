...
function boot() {
  ...
  return { canvas, doc, box, get, banners };
}
...
function showPanel(page, link, opts = {}) {
  ...
  `);
  // marshal through JSON: values built inside the vm context are foreign
  // objects to the host's assert.deepEqual (different Array/Object realm)
  return {
    applied: () => JSON.parse(page.get("JSON.stringify(globalThis.__applied)")),
    fetched: () => JSON.parse(page.get("JSON.stringify(globalThis.__fetched)")),
  };
}
...
test("hide closes the window; Escape hides it too", () => {
  const page = boot();
  page.get("LinkPanel.attach(__canvas)");
  showPanel(page, plainLink);
  page.get("LinkPanel.hide()");
